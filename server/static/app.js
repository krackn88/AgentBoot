...
const sensorConfigFile = $("#sensorConfigFile");
const comboFileName = $("#comboFileName");
const proxyFileName = $("#proxyFileName");
const sensorConfigName = $("#sensorConfigName");
const sensorConfigRow = $("#sensorConfigRow");
...
function updateModeUi() {
  fullBootstrap = modeBootstrap.checked;
  autoSensors = modeCapture.checked;
  sensorConfigRow.hidden = !autoSensors;
}
...
function loadSmokeCombo() {
  try {
    const saved = localStorage.getItem("swa_smoke_combo");
...
function saveSmokeCombo() {
  try {
    localStorage.setItem("swa_smoke_combo", smokeCombo.value);
  } catch (_) {}
}
...
async function loadSession() {
  ...
  try {
    ...
    fullBootstrap = data.full_bootstrap !== false;
    autoSensors = Boolean(data.auto_sensors);

    modeBootstrap.checked = fullBootstrap;
    modeCapture.checked = autoSensors;
    updateModeUi();
...
function updateResumeHint() {
  ...
  const mode = fullBootstrap ? "full bootstrap" : "capture template";
  resumeHint.textContent = `${comboInfo}${checked} · ${mode}`;
}

modeBootstrap.addEventListener("change", () => { updateModeUi(); scheduleSave(); });
modeCapture.addEventListener("change", () => { updateModeUi(); scheduleSave(); });
...
sensorConfigFile.addEventListener("change", () => {
  const file = sensorConfigFile.files[0];
  sensorConfigName.textContent = file ? file.name : "No file selected";
});
...
function updateStatus(data) {
  ...
  statRetries.textContent = String(data.retries || 0);
  ...
  if (data.retries) parts.push(`${data.retries} retry`);
...
startBtn.addEventListener("click", async () => {
  ...
  if (autoSensors && !sensorConfigFile.files[0]) {
    const hasStoredConfig = sensorConfigName.textContent.includes("on server");
    if (!hasStoredConfig) {
      showToast("Upload sensor_config.json for capture mode");
      return;
    }
  }
